...
public class DoorController {
    ...
    private static final int MAX_ELEMENTS_IN_RESPONSE = 10;
    ...
    public ResponseEntity<List<HeartbeatView>> getHeartbeat(@PathVariable("device_id") String deviceId) {
        return ResponseEntity.ok(heartbeatRepository.findByDeviceIdOrderByTimestampDesc(deviceId).stream()
                ...
                .limit(MAX_ELEMENTS_IN_RESPONSE)
                .collect(Collectors.toList()));
    }

    @GetMapping("/home/security/door/heartbeats")
    public ResponseEntity<List<HeartbeatView>> getHeartbeats() {
    ...
    public ResponseEntity<List<DoorStatusView>> getDoorStatus(@PathVariable("device_id") String deviceId) {
    ...
    public ResponseEntity<List<DoorStatusView>> getDoorStatuses() {
        return ResponseEntity.ok(doorStatusRepository.findAll().stream()
                .map(this::createDoorStatus)
                .limit(MAX_ELEMENTS_IN_RESPONSE)
                .collect(Collectors.toList()));
    }
